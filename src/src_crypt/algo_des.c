...
char *encrypt(char *pt, char *rkb, char *rk)
{ 
    ...
    for(int i=0; i<16; i++){ 
        //Expansion D-box 
        string right_expanded= permute(right, g_exp_d, 48); 
    ...
    //Final Permutation 
    string cipher= bin2hex(permute(combine, g_final_perm, 64)); 
    return cipher;
} 